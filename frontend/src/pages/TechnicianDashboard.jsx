...
const TechnicianDashboard = () => {
  ...
  const fetchTickets = async () => {
    try {
      setLoading(true);
      const response = await api.get('/tickets?assignedTo=TechUser');
  ...
  const handleUpdateTicket = async (e) => {
    ...
    try {
      await api.put(`/tickets/${selectedTicket.id}`, {
        status: updateStatus,
        assignedTo: 'TechUser', // Assigning to current tech
  ...
  if (role !== 'technician' && role !== 'admin') {
    return <div className="p-8 text-center text-red-500">Access Denied. Technician Role Required.</div>;
  }

  return (
    <div className="flex flex-col gap-6 animate-in fade-in">
      <h2 className="text-2xl font-bold">Technician Dashboard</h2>
      <p className="text-gray-500 text-sm">Review, assign, and update incident tickets.</p>
      ...
      {selectedTicket && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg">
             <h3 className="text-xl font-bold mb-4">Update Ticket #{selectedTicket.id.substring(20)}</h3>
