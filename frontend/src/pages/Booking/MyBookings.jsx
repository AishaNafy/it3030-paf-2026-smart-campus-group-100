  ...
  const handleSave = async () => {
    ...
    try {
      const res  = await fetch(`${API}/${booking.id}`, {
        ...
        body: JSON.stringify({ ...form, studentId: form.studentId.trim().toUpperCase(),
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden">
        <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100" style={{ backgroundColor: '#dafff7' }}>
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              ...
              &nbsp;· Saving resets status to <strong>PENDING</strong>.
            </p>
          </div>
        </div>
        {error && (
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className={labelClass}>ID No</label>
              <input type="text" name="studentId" value={form.studentId} onChange={handleChange} className={inputClass} /></div>
            <div><label className={labelClass}>Location</label>
              <input type="text" name="location" value={form.location} onChange={handleChange} className={inputClass} /></div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div><label className={labelClass}>Date</label>
              <input type="date" name="date" value={form.date} onChange={handleChange} className={inputClass} /></div>
            <div><label className={labelClass}>Start Time</label>
              <input type="time" name="startTime" value={form.startTime} onChange={handleChange} className={inputClass} /></div>
            <div><label className={labelClass}>End Time</label>
              <input type="time" name="endTime" value={form.endTime} onChange={handleChange} className={inputClass} /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className={labelClass}>Attendees</label>
              <input type="number" name="attendees" value={form.attendees} onChange={handleChange} min="1" className={inputClass} /></div>
          </div>
          <div><label className={labelClass}>Purpose</label>
            <textarea name="purpose" value={form.purpose} onChange={handleChange}
              className={`${inputClass} h-20 resize-none`} /></div>
        </div>
        <div className="px-6 pb-5 flex justify-end gap-2">
          <button onClick={onCancel}
            className="px-5 py-2.5 rounded-xl text-sm font-semibold border border-gray-200 text-gray-600 hover:bg-gray-50 transition">
          ...
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            ...
            {b.status === 'APPROVED' && (
              <button onClick={() => {
                  ...
                  if (confirmed) onApprove(b.id, 'CANCELLED');
                }} disabled={busy}
        ...
        {expanded && (
          <div className="border-t border-gray-100 px-5 py-4 bg-[#dafff7]/20 rounded-b-2xl">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm text-[#010101]">
              ...
              {b.createdAt && <div><p className="text-xs text-gray-400 uppercase font-semibold mb-0.5">Submitted At</p>
                <p>{new Date(b.createdAt).toLocaleString()}</p></div>}
              {b.updatedAt && <div><p className="text-xs text-gray-400 uppercase font-semibold mb-0.5">Last Updated</p>
...
const MyBookings = () => {
  ...
  const [deleteTarget,  setDeleteTarget]  = useState(null); // id awaiting delete confirm
  const [logKey,        setLogKey]        = useState(0);    // increment to re-mount DeletedLog
  ...
  const handleDelete  = (id) => setDeleteTarget(id); // opens delete modal
  ...
  const confirmDelete = async (reason) => {
    ...
    try {
      ...
      fetchBookings();
      setLogKey(k => k + 1); // refresh deleted log
  ...
  return (
    <div className="min-h-full">
      ...
      {/* Deleted bookings log — always rendered at bottom */}
